...
class Storage
{
     ...
     * 
     * @param string $path
     *            The full path to where archives are stored
     */
     ...
     * @param array $data
     *            The data the driver is validating
    ...
    public function validateDriver(\mithra62\Validate $validate, $driver, array $data, array $locations = array())
    {
        ...
        if (isset($storage_drivers[$driver]) && $storage_drivers[$driver]['obj'] instanceof Storage\StorageInterface) {
            $validate = $storage_drivers[$driver]['obj']->setServices($this->getServices())
                ->setExistingStorageLocations($locations)
                ->validateSettings($validate, $data);
        }
     ...
     * 
     * @param string $driver
     *            The short name of the Storage Driver
     */
     ...
     * @param array $locations
     *            An array of existing Locations to determine what Drivers to use and send the file to where
     * @param string $type            
     ...
     * 
     * @param array $backup
     *            The backup details array we want to remove
     * @param Details $details
     *            The Details object
     * @param bool $cleanup
     *            Whether this is a cleanup removal, in which case the "storage_location_include_prune" setting comes into play
     * @return boolean
     */
