...
class Backups
{
     ...
     * 
     * @param bool $auto_threshold
     *            Whether to compile for viewing or use
     * @param array $backups
     *            The existing backups on the system to use for determining data
     ...
     * 
     * @param string $backup
     *            The full path to the backup we want details on
    ...
    public function getBackupData($backup)
    {
        ...
        foreach ($locations as $location) {
            if (isset($backup_data['storage'][$location['location_id']])) {
                $location['obj'] = $this->getStorage()
                    ->getDriver($location['storage_location_driver'])
